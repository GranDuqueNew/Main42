...
/*   Updated: 2021/08/23 21:28:42 by jduque-p         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strnstr(const char *strBig, const char *strlittle, size_t len)
{
	...
	while (strBig[cnt1] != '\0' && cnt1 < len)
	{
		...
		{
			while (strlittle[cnt2] != '\0'
				&& strBig[cnt1 + cnt2] == strlittle[cnt2]
				&& cnt1 + cnt2 < len)
			{
...
int main()
{
// La función *ft_strnstr(const char *strBig, const char *strlittle, size_t len):
// Localice la subcadena, donde no se buscan más de los caracteres 'len'.
// Encuentra la primera aparición de la subcadena 'strlittle' en la 
// cadena 'strBig'. Los bytes nulos de terminación ('\ 0') no se comparan.
